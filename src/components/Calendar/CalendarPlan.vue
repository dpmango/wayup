      ...
      v-col(
          ...
          .buttons-wrap(data-app)
            ...
            .summury-plan__modal-wrap(:class="isActiveModal ? 'm-show' : 'm-hide'")
              .summury-plan__modal(v-draggable)
                ...
                .summury-plan__block
                  .summury-plan__block
                    template(v-for="part in plan.eventParts")
                      .summury-plan__block-title(v-if="part.exercisesEvent.length") {{ part.name }}
                      ul.summury-plan__list(v-if="part.exercisesEvent.length")
                        li.summury-plan__item(v-for="(ex, index) in part.exercisesEvent")
                          .summury-plan__num {{ index + 1 }}
                          .summury-plan__desc {{ ex.exercise.title }}
                          .labels
                            base-label(
                              :label='ex.exercise.duration + " мин"'
                              color='#F1F3F9'
                              textColor="#000000"
                            )
                            base-label(
                              :label='ex.exercise.loadIntensity[0].name'
        ...
        v-row
          ...
          v-col(
            ...
            .div(
              :class="!isActiveEdit ? 'd-none' : 'd-block'"
              v-if="role == trainerRole"
            )
